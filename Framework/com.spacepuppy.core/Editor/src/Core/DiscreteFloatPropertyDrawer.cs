...
namespace com.spacepuppyeditor.Core
{
    ...
    public class NullableIntPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                prop_hasvalue.boolValue = hasvalue;
                prop_value.intValue = hasvalue ? NormalizeBasedOnAttribs(0) : 0;
            }

            if (hasvalue)
            {
                EditorGUI.BeginChangeCheck();
                int value = EditorGUI.IntField(r1, prop_value.intValue);
                if (EditorGUI.EndChangeCheck())
                {
                    value = NormalizeBasedOnAttribs(value);
                    prop_value.intValue = value;
                }
            }
            else
            {
                prop_value.intValue = 0;
                EditorGUI.BeginChangeCheck();
                string soutput = EditorGUI.DelayedTextField(r1, longlabel, GUI.skin.textField);
                if (EditorGUI.EndChangeCheck())
                {
                    if (int.TryParse(soutput, out int output))
                    {
                        prop_hasvalue.boolValue = true;
                        prop_value.intValue = NormalizeBasedOnAttribs(output);
                    }
                }
            }
        }
        ...
        void GetLabels(out string shortlabel, out string longlabel)
        {
            var attrib = this.fieldInfo?.GetCustomAttribute<NullableInt.LabelAttribute>();
            shortlabel = attrib?.ShortLabel ?? "null";
            longlabel = attrib?.LongLabel ?? "NULL";
        }

        private int NormalizeBasedOnAttribs(int value)
        {
            var attribs = this.fieldInfo?.GetCustomAttributes(typeof(NullableInt.ConfigAttribute), false) as NullableInt.ConfigAttribute[];
            if (attribs == null) return value;
