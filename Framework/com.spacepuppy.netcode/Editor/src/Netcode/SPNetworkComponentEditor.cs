...
namespace com.spacepuppyeditor.Netcode
{
    ...
    internal class AttemptedInjectNetworkBehaviourEditor : NetworkBehaviourEditor
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            if (SpacepuppySettings.ProjectPrefs.GetBool("SPNetworkComponent.PromptNetworkObject", true))
            {
    ...
    [InitializeOnLoad]
    [CustomEditor(typeof(SPNetworkComponent), true)]
    [CanEditMultipleObjects]
    public class SPNetworkComponentEditor : SPEditor
    {

        private NetworkBehaviourVariableDrawer _networkVariableDrawer;

        static SPNetworkComponentEditor()
        {
            SpacepuppySettingsWindow.DrawExtraEditorSettings += () =>
            {
                EditorGUI.BeginChangeCheck();
                bool promptNetworkObject = EditorGUILayout.ToggleLeft("NetworkBehaviours Prompt To Add NetworkObject", SpacepuppySettings.ProjectPrefs.GetBool("SPNetworkComponent.PromptNetworkObject", true));
                if (EditorGUI.EndChangeCheck())
                {
                    SpacepuppySettings.ProjectPrefs.SetBool("SPNetworkComponent.PromptNetworkObject", promptNetworkObject);
                }
            };
        }
        ...
        protected override void OnEnable()
        {
            ...
            _networkVariableDrawer = new NetworkBehaviourVariableDrawer()
            {
    ...
    internal class NetworkBehaviourVariableDrawer
    {
        ...
        private void RenderNetworkVariable(int index)
        {
            ...
            if (value == null)
            {
                ...
                var networkVariable = (NetworkVariableBase)System.Activator.CreateInstance(fieldType, true);
        ...
        private void RenderNetworkVariableValueType<T>(int index) where T : unmanaged
        {
            ...
            if (behaviour.IsBehaviourEditable())
            {
                ...
                {
                    val = EditorGUILayout.EnumPopup(name, (System.Enum)val);
                }
                else
                {
                    EditorGUILayout.LabelField("Type not renderable");
    ...
    internal static class NetworkBehaviourExtensions
    {
        public static bool IsBehaviourEditable(this NetworkBehaviour behaviour)
        {
            if (!behaviour) return false;
            return !behaviour.NetworkObject ||
                behaviour.NetworkObject.NetworkManager == null ||
                behaviour.NetworkObject.NetworkManager.IsListening == false ||
                behaviour.NetworkObject.NetworkManager.IsServer;
        }
    }
